...
// Platform detection using predefined macros
	...
	#ifdef _WIN64
		/* Windows x64  */
		#define SPIRIT_PLATFORM_WINDOWS
	#else
		/* Windows x86 */
	 ...
	 * and not some other Apple platform */
		...
		#define SPIRIT_PLATFORM_IOS
		#error "IOS is not supported!"
	#elif TARGET_OS_MAC == 1
		#define SPIRIT_PLATFORM_MACOS
 ...
 * it has __linux__ defined */
	...
	#define SPIRIT_PLATFORM_ANDROID
	#error "Android is not supported!"
#elif defined(__linux__)
	#define SPIRIT_PLATFORM_LINUX
...
#else
	#define SPIRIT_ASSERT(x, ...)
	#define SPIRIT_CORE_ASSERT(x, ...)
#endif
