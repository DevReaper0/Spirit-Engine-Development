...
#define AUDIO_LOG(x) SPIRIT_INFO(((std::string)"[Spirit Audio] ") + (std::string(x)))
	...
	static AudioFileFormat GetFileFormat(const std::string& filename)
	{
		...
		if (extension == ".aiff")  return AudioFileFormat::Aiff;
		if (extension == ".midi")  return AudioFileFormat::Midi;
		if (extension == ".cda")  return AudioFileFormat::Cda;
		if (extension == ".spiritaudio")  return AudioFileFormat::Spirit;
	...
	AudioSource Audio::LoadAudioSourceOgg(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			AUDIO_LOG("  File Info - " + filename + ":");
			AUDIO_LOG("  Channels: " + (std::to_string(channels)));
			AUDIO_LOG("  Sample Rate: " + (std::to_string(sampleRate)));
			AUDIO_LOG("  Expected size: " + (std::to_string(bufferSize)));


		}

		// TODO: Replace with SpiritEngine::Buffer
		...
		if (s_DebugLog)
			AUDIO_LOG("  Read " + (std::to_string(size)) + " bytes");
		...
		if (alGetError() != AL_NO_ERROR)
			AUDIO_LOG("Failed to setup sound source");
	...
	AudioSource Audio::LoadAudioSourceMP3(const std::string& filename)
	{
		...
		if (s_DebugLog)
		{
			...
			AUDIO_LOG("  Channels: " + (std::to_string(channels)));
			AUDIO_LOG("  Sample Rate: " + (std::to_string(sampleRate)));
			AUDIO_LOG("  Size: " + (std::to_string(size)) + " bytes");

			auto [mins, secs] = result.GetLengthMinutesAndSeconds();
			AUDIO_LOG("  Length: " + (std::to_string(mins)) + "m" + (std::to_string(secs)) + "s");
		}
	...
	AudioSource Audio::LoadAudioSourceAiff(const std::string& filename)
	{
		mp3dec_file_info_t info;
		int loadResult = mp3dec_load(&s_Mp3d, filename.c_str(), &info, NULL, NULL);
		uint32_t size = info.samples * sizeof(mp3d_sample_t);
	...
	AudioSource Audio::LoadAudioSourceMidi(const std::string& filename)
	{
		mp3dec_file_info_t info;
		int loadResult = mp3dec_load(&s_Mp3d, filename.c_str(), &info, NULL, NULL);
		uint32_t size = info.samples * sizeof(mp3d_sample_t);
	...
	AudioSource Audio::LoadAudioSourceCda(const std::string& filename)
	{
		mp3dec_file_info_t info;
		int loadResult = mp3dec_load(&s_Mp3d, filename.c_str(), &info, NULL, NULL);
		uint32_t size = info.samples * sizeof(mp3d_sample_t);
	...
	AudioSource Audio::LoadAudioSourceSpirit(const std::string& filename)
	{
		mp3dec_file_info_t info;
		int loadResult = mp3dec_load(&s_Mp3d, filename.c_str(), &info, NULL, NULL);
		uint32_t size = info.samples * sizeof(mp3d_sample_t);
	...
	static void PrintAudioDeviceInfo()
	{
		if (s_DebugLog)
		{
			AUDIO_LOG("Audio Device Info:");
			AUDIO_LOG("  Name: " + s_AudioDevice->DeviceName);
			AUDIO_LOG("  Sample Rate: " + s_AudioDevice->Frequency);
			AUDIO_LOG("  Max Sources: " + s_AudioDevice->SourcesMax);
			AUDIO_LOG("    Mono: " + s_AudioDevice->NumMonoSources);
			AUDIO_LOG("    Stereo: " + s_AudioDevice->NumStereoSources);
		}
	}
	...
	AudioSource Audio::LoadAudioSource(const std::string& filename)
	{
		...
		switch (format)
		{
			...
			case AudioFileFormat::Aiff:  return LoadAudioSourceAiff(filename);
			case AudioFileFormat::Midi:  return LoadAudioSourceMidi(filename);
			case AudioFileFormat::Cda:  return LoadAudioSourceCda(filename);
			case AudioFileFormat::Spirit:  return LoadAudioSourceSpirit(filename);
		}
